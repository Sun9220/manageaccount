...
import { deleteMembership, getAccountRelatedLogs, listMembershipsByAccount } from '../services/data';
...
export function AccountDetail({ accountId, refreshKey, onRefresh }: { accountId: string; refreshKey: number; onRefresh: () => void }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      getAccountRelatedLogs(accountId),
      db.apps.toArray(),
          ...
          {items.map((m) => (
            <div key={m.id} className="rounded border p-2 text-sm">
              <div className="mb-1 flex items-center gap-2 text-slate-800">
                <AppIcon icon={apps.find((a) => a.id === m.appId)?.icon} />
                <span>{mapName.appById[m.appId]} - {mapName.planById[m.planTemplateId]} - {mapName.durById[m.durationTemplateId]}</span>
              </div>
              <div className="text-slate-600">状态 {m.status} | 开始 {formatDateTime(m.startAt)} | 到期 {formatDateTime(m.endAt)} | 创建 {formatDateTime(m.createdAt)} | 更新 {formatDateTime(m.updatedAt)}</div>
